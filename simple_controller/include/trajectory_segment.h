...
public:
  ...
  virtual double get_length() const = 0; //返回段长度
  /// \ returns curvature of segment
  virtual double get_curvature() const = 0; //返回当前段曲率
  /// \ returns point of trajectory for the given length
  virtual tf::Vector3 get_point(double point_len) const = 0; //返回从开头开始给定长度的线段的点（向量 x、y、z）
  ...
  virtual double get_point_length(double x, double y) const = 0; //返回到最近的点距离线段长度
  /// \ returns distance to nearest trajectory point w r t direction (normal error)
  virtual double get_point_distance(double x, double y) const = 0; //返回到最近的点距离
  virtual ~TrajectorySegment(){}
};

class CircularSegment: public TrajectorySegment //каждый сегмент
{
protected:
  /// \ 1/R
  double curvature;
...
class LinearSegment : public TrajectorySegment //каждый сегмент
{
protected:
  tf::Vector3 x0;
  tf::Vector3 v;
